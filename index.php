    ...
    foreach ($data['issues'] as $issue) {
      ...
      foreach ($issue['custom_fields'] as $index => $field) {
        ...
        if ($field["name"] == "Content Owner Name") {
          ...
          if (!in_array($field["value"], $cc)) {
            $cc[] = $field["value"];
          }
          if (!in_array($field["value"], $cc2)) {
            $cc2[] = $field["value"];
          }
      ...
      if (isset($name, $to, $expiry_date) && (isset($removed) && !$removed)) {
        ...
        if ($sendActionNeeded && isset($issue['assigned_to'])) {

          $gulf_manager = $issue['assigned_to']["id"];
          ...
          if (!empty($user_data)) {
            if (!in_array($user_data['user']['mail'], $cc)) {
